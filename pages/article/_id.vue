							...
							<div id="respond">
								...
								<form class="row"  v-on:submit.prevent="addcomment(article.id)" action="#" method="post" id="commentform">
									<div class="col-md-4 form-group">
										<label for="author">Prénom ou surnom</label>
...
export default {
	...
	methods:{
		addcomment : async function(id){
			...
			const req = await this.$axios.post(`/coments`,{author:this.authorToPost,text:this.messageToPost,article:id})
    ...
    async asyncData({$axios,route,params}) {
        console.log(params)
      const post = await $axios.get(`/articles/`+params.id)
      ...
      const previousarticle = await $axios.get(`/articles?${query}&_sort=published_at:ASC&_limit=1`)
      const nextarticle = await $axios.get(`/articles?${query2}&_sort=published_at:DESC&_limit=1`)
