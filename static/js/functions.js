...
(function ($) {
  ...
  SEMICOLON.header = {
    ...
    menuInvert: function (subMenuEl) {
      const submenus = subMenuEl || $('.mega-menu-content, .sub-menu-container, .top-links-section')

      submenus.children().css({ display: 'block' })
      submenus.css({ display: 'block' })
      submenus.each(function (index, element) {
        const $menuChildElement = $(element)
					let menuChildOffset = $menuChildElement.offset()
					let menuChildWidth = $menuChildElement.width()
    ...
    menufunctions: function () {
      const menuItemSubs		= $('.menu-item:has(.sub-menu-container)')
				let menuItemSubsLinks	= menuItemSubs.children('.menu-link')
				let submenusT			= '.mega-menu-content, .sub-menu-container'
				let submenus			= $(submenusT)
				let menuItemT			= '.menu-item'
				let subMenuT			= '.sub-menu'
				let menuSpeed			= primaryMenu.attr('data-trigger-speed') || 200
				let subMenuTriggerT		= '.sub-menu-trigger'
				let menuItemTrigger
      ...
      if (($body.hasClass('overlay-menu') && primaryMenu.hasClass('on-click')) && ($body.hasClass('device-xl') || $body.hasClass('device-lg'))) {
        ...
        menuItemTrigger.off('click').on('click', function (e) {
          ...
          const subMenuTriggerEl = triggerEl.parent(menuItemT).children(subMenuTriggerT)
    ...
    fullWidthMenu: function () {
      ...
      const headerWidth = $('.mega-menu:not(.mega-menu-full):not(.mega-menu-small) .mega-menu-content').parents('.header-row').width()
      ...
      if ($header.find('.header-row').length > 1) {
        const megaMenuContent	= $('.menu-container > .mega-menu:not(.mega-menu-small) .mega-menu-content').eq(0)
					let offset			= $headerWrap.outerHeight() - megaMenuContent.parents('.header-row').outerHeight()
					let css				= '.menu-container > .mega-menu:not(.mega-menu-small) .mega-menu-content { top: calc( 100% - ' + offset + 'px ); }'
  ...
  SEMICOLON.slider = {
    ...
    sliderDimensions: function () {
      let parallaxElHeight	= $sliderParallaxEl.outerHeight()
				...
				let slInner				= $sliderParallaxEl.find('.slider-inner')
				let slSwiperW			= $slider.find('.swiper-wrapper')
				let slSwiperS			= $slider.find('.swiper-slide').first()
				let slFlexHeight		= $slider.hasClass('h-auto') || $slider.hasClass('min-vh-0')
    ...
    headerSchemeChanger: function (activeSlide, onWinLoad) {
      if (activeSlide.length > 0) {
        let darkExists = false
					let oldClassesArray; let noOfOldClasses
  ...
  SEMICOLON.portfolio = {
    ...
    revealDesc: function () {
      ...
      $portfolioReveal.each(function () {
        const element			= $(this)
					let elementItems	= element.find('.portfolio-item')
  ...
  SEMICOLON.documentOnReady = {
    ...
    windowscroll: function () {
      ...
      const pageMenuWrap	= $pagemenu.find('#page-menu-wrap')
				let offset			= $headerWrap.outerHeight()
				let head			= document.head || document.getElementsByTagName('head')[0]
				let style			= document.createElement('style')
				let css
  ...
  const $window = $(window)
		...
		let mobileStickyLogoH = $header.attr('data-mobile-sticky-logo-height') || Number(stickyLogoH)
		...
		let $headerWrapClone = ''
		let initialHeaderWrapHeight = $headerWrap.outerHeight()
		let $headerRow = $headerWrap.find('.header-row:eq(0)')
		let $content = $('#content')
		let $footer = $('#footer')
		...
		let defaultLogoWidth = defaultLogo.find('img').outerWidth()
		...
		let $onePageMenuEl = $('.one-page-menu')
		...
		let $topSocialEl = $('#top-social').find('li')
